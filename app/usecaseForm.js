...
import {USER ACTION} from './constants'; import {execute} from './network';
...
class EndPoint extends React.Component {
	render() {
		...
		return (<FormGroup>
			...
			<Col sm={1} style={{padding: '0px'}}>
				<InputGroup>
					<FormControl componentClass='select' placeholder='http/https'
							onChange={this.props.handleChange} name={epId + 'ssl'}
							value={ep[epId + 'ssl']}›
			...
			<Col sm={1} style={{padding: '0px'}}> 
				<InputGroup> 
					<FormControl type='text' value={ep[epId + 'port']} placeholder='Port' 
							name={epId + 'port'} onChange={this. props. handleChange}/> 
						<InputGroup.Addon>/</InputGroup.Addon> 
				</InputGroup> 
			</Col>
            <Col sm={4} style={{padding: '0px', paddingRight: '15px'}}> 
            	<FormControl type='text' value={ep[epId + 'path']} name={epId + 'path'} 
            			placeholder='End point path' onChange={this. props. handleChange}/> 
...
class UseCaseForm extends React.Component {
	constructor(props) {
		super(props);
		this.state = { summary: '', desc: '', payload: '', expected: '', BAndTComp: 'BAE', responseType: 'OBJECT',
			bepssl: false, bephost: '', bepport: '', beptype: 'GET', beppath: '',
			tepssl: false, tephost: '', tepport: '', teptype: 'GET', teppath: '' 
		}; 
	...
	handleSave(e) { 
		e.preventDefault();
 		let currState = this.state.clone();
 		let reqObj = {summary: currState.summary, desc: currState.desc,
 					BAndTComp: currState.BAndTComp, responseType: currState.responseType,
 					bepssl: currState.bepssl === 'true', bephost: currState.bephost,
 					bepport: parseInt(currState.bepport), beppath: currState.beppath
 					};
	    ...
	    if (currState.BAndTComp === 'BAT') {
	    	reqObj = Object.assign({}, reqObj, {
	    						tepssl: currState.tepssl === 'true',
	    						tephost: currState.tephost,
	    						tepport: parselnt(currState.tepport),
	...
	render() { 
		...
		let expected = (<FormGroup controlId='expected'> 
			...
			<Col sm={11}><FormControl componentClass='textarea' value={this.state.expected} 
    	...
    	let payload = (<div/>);
 		if (this.state.beptype === 'POST') { 
			payload = (<FormGroup controlId='payload' validationState={this.inputJsonValidation()}> 
				...
				<Col sm={11}> <FormControl componentClass='textarea' value={this.state.payload} placeholder='Input Json' 
								name='payload' onChange={this.handleChange}/><FormControl.Feedback/>
				</Col></FormGroup>);
    	...
    	return (<div style={{paddingLeft: '2.5%', paddingRight: '2.5%'}}> 
    				<Form horizontal> 
    					<FormGroup controlId='summary' validationState={this.summaryValidation()}>
    						...
    						<Col sm={11}><FormControl type='text' value={this.state.summary} placeholder='Summary'
						...
						<FormGroup controlId='desc' validationState={this.descriptionValidation0}> 
							...
							<Col sm={11}><FormControl componentClass='textarea' value={this.state.desc} placeholder='Description' 
								name='desc' onChange={this.handleChange}/><FormControl.Feedback/>
							</Col> 
						</FormGroup> 
						
						<FormGroup controlId='BAndTComps'> 
							<Col componentClass={ControlLabel} sm={1}>Comparison Type</Col> 
							<Col sm={11}><FormControl componentClass='select' placeholder='Comparison Type' 
